...
#SBATCH --cpus-per-task=16 #8 #16                                # Ask for 2 CPUs
#SBATCH --gres=gpu:a100l:1                                     # Ask for 1 GPU
#SBATCH --mem=256G #48G #256G   #24G for Resnet18                                        # Ask for 10 GB of RAM
...
python source/structured_baseline.py dataset=$SLURM_TMPDIR/imagenet2012 architecture='resnet50' training_steps=500456 report_freq=2000 record_freq=500 full_ds_eval_freq=600000 size=64 with_bn=True lr_schedule=warmup_piecewise_decay "lr_decay_steps='(30, 70, 90)'" lr_decay_scaling_factor=0.1 normalize_inputs=True info=Resnet50_structured_imgnet_srigl_momentum pruning_criterion=crop-it optimizer=momentum9w wd_param=0.0001 lr=0.1 train_batch_size=256 eval_batch_size=256 death_batch_size=256 augment_dataset=True gradient_clipping=False noisy_label=0.0 label_smoothing=0.1 regularizer=None activation=relu save_wanda=False modulate_target_density=False pruning_steps=5 pruning_density=0.34 preempt_handling=True init_seed=31
wait $!

# for resnet, ~80% param sparsity --> pruning density = 0.4, 0.45, 0.5
#python source/structured_baseline.py dataset=$SLURM_TMPDIR/imagenet2012 architecture='resnet50' training_steps=500456 report_freq=2000 record_freq=500 full_ds_eval_freq=600000 size=64 with_bn=True lr_schedule=warmup_piecewise_decay "lr_decay_steps='(30, 70, 90)'" lr_decay_scaling_factor=0.1 normalize_inputs=True info=Resnet50_structured_imgnet_srigl_adam pruning_criterion=snap optimizer=adamw wd_param=0.0001 lr=0.001 train_batch_size=256 eval_batch_size=256 death_batch_size=256 augment_dataset=True gradient_clipping=False noisy_label=0.0 label_smoothing=0.1 regularizer=None activation=relu save_wanda=False modulate_target_density=False pruning_steps=5 pruning_density=0.34 preempt_handling=True init_seed=33
#wait $!
