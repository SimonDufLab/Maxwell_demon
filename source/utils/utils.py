...
def load_imagenet_tf(dataset_dir: str, split: str, *, is_training: bool, batch_size: int,
   ...
   #  )
    ...
    if 'vit' in dataset:
        def load_routine(_ds, _training, _bs):
            ds1 = _ds
            if _training:
                ds1 = ds1.shuffle(250000, seed=0, reshuffle_each_iteration=True)
            ds1 = ds1.map(Partial(resize_tf_dataset, dataset=dataset, is_training=_training),
                          num_parallel_calls=tf.data.AUTOTUNE)
            ds1 = ds1.batch(_bs)
            if _training and data_augmentation:  # Only ds1 takes into account 'is_training' flag
                ds1 = ds1.map(augment_train_imagenet_dataset_vit, num_parallel_calls=tf.data.AUTOTUNE)
            else:
        ...
        def load_routine(_ds, _training, _bs):
            ...
            ds1 = ds1.map(Partial(resize_tf_dataset, dataset=dataset, is_training=_training),
                          num_parallel_calls=tf.data.AUTOTUNE)
            if _training and data_augmentation:  # Only ds1 takes into account 'is_training' flag
                ds1 = ds1.map(augment_train_imagenet_dataset_res50, num_parallel_calls=tf.data.AUTOTUNE)
    ...
    if other_bs:
        all_ds = [load_routine(ds, is_training, batch_size)]
        for bs in other_bs:
            all_ds.append(load_routine(ds, False, bs))
        ...
        ds = load_routine(ds, is_training, batch_size)
...
def load_cifar10_tf(split: str, is_training, batch_size, other_bs=None, subset=None, transform=True, cardinality=False,
                    noisy_label=0, permuted_img_ratio=0, gaussian_img_ratio=0, augment_dataset=False,
                    normalize: bool = False, reduced_ds_size: Optional[int] = None, dataset: str = "cifar10"):
...
class RunState(TypedDict):  # Taken from https://docs.mila.quebec/examples/good_practices/checkpointing/index.html
    ...
    reset_counter: Optional[dict]
    reset_tracker: Optional[dict]
...
def checkpoint_exp(run_state: RunState, params, state, opt_state, curr_epoch: int, curr_step: int,
                   ...
                   best_acc, best_params_count, best_total_neurons, training_time, reset_counter, reset_tracker,
    ...
    run_state["reset_counter"] = reset_counter
    run_state["reset_tracker"] = reset_tracker
